	...
	<view class="register">
		...
		<view class="content">
			...
			<view class="border-bottom">
			  <view class="title">邀请人手机号</view>
			  <view><input class="input-info" maxlength="11" v-model="mobile"  placeholder="须填邀请人注册时手机号" /></view>
			</view>
			
			
			<button type="primary" @tap="goConfirm()" style="margin:40px 20px 20px 20px" size="default">确认</button>
			<button type="default" @tap="goSkip()"  style="margin:20px" size="default">跳过</button>
		</view>
	...
	export default {
		...
		methods: {
			...
			goConfirm() {
				...
				if (!token) {
					this.$api.msg("缺少token");
					return;
				}
				uni.request({
					url:this.$url.apiUrl() + "inviter",
					method:"POST",
					dataType:"json",
