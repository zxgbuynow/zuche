	...
	<view class="register">
		...
		<view class="content">
			...
			<view class="border-bottom">
			  ...
			  <view><input class="input-info" maxlength="11" v-model="mobile"  placeholder="手机号" /></view>
			</view>
			<view class="border-bottom">
			  <view class="title-vcode">验证码</view>
			  <view><input class="input-vcode" v-model="verCode"  placeholder="6位数字" /></view>
			  <button class="send-vcode" :disabled="disabled" @tap="getVerCode()"><text style="color: #ccc;margin-right: 3px;">|</text> {{countdown}} <text v-show="timestatus" style="background: #FFFFFF;">秒重获</text></button>
			</view>
			
			<button type="primary" @tap="startReg()" style="margin:20px" size="default">登入/注册</button>
		</view>
		...
		<view style="text-align: center;font-size: 14px;color: #0081FF;">关于</view>
		<view style="text-align: center;font-size: 14px;color: #C0C0C0;">上海盔甲网络科技有限公司</view>
	</view>
</template>
				...
				uni.request({
					...
					success: (res) => {
						if(res.data.status==0) {
							...
							uni.setStorage({
								key:"doen",
								data:res.data.data.done,
								success() {
									console.log("success");
								}
							});
							if(res.data.data.is_new == 1) { //新用户 跳转邀请页面
								uni.navigateTo({
									url:"invitation",
								})
							}else {
